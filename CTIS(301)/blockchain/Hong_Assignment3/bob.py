...
def generateBlock(position):
	...
	cond = True
	count = count + 1
	fr = open(f'bob/block{count-1}.json', "r")
	preblk = fr.read()
	fr.close()
	hashValue = hashlib.sha256(preblk.encode()).hexdigest()
	
	while(cond):
		block = json.dumps({'Block number': count, 'Hash': hashValue, 'Nonce': nonce, 'Transaction': ["Bob", position]}, sort_keys=False,indent=4, separators=(',', ': '))
		hashnonce = hashlib.sha256(block.encode()).hexdigest()
		if int(hashnonce[0:4],16) < 2^244:
			cond = False
		nonce = nonce + 1
		
	fr = open(f'bob/block{count}.json', "w")
	fr.write(block)
	fr.close()
...
def generateRandomBlockList():
	...
	for key,value in board.items():
		if value == 1:
			...
			boardlist.append(key)
	return boardlist

def selectRandomPlacement(tempboard):
	tempchoice = random.choice(tempboard)
	board[tempchoice] = 1
	print("----------------------------------------\nTic Tac Toe: ", board, "\n----------------------------------------")
	return tempchoice
...
def writeBlock(block):
	...
	if(result):
		fw = open(f'bob/block{count}.json',"w")
		fw.write(block)
		fw.close()
		return formatblock
	else:
...
class MySubscribeCallback(SubscribeCallback):
	...
	def status(self, pubnub, status):
		if status.category == PNStatusCategory.PNConnectedCategory:
			...
			print("----------------------------------------\nTic Tac Toe: ", board, "\n----------------------------------------")
			greeting = {"sender": "Bob", "message": "Hello from Bob", "turn": 0} # To check response of first player
	...
	def message(self, pubnub, message):
		...
		if tempmessage["sender"] == "Alice" and tempmessage["turn"] == 0: # If Alice is first player, Alice will generate first block only after Bob sent his first message and vice versa
			...
			messagetosend = {"sender": "Bob", "message": tempblock, "turn": count}
		...
		elif tempmessage["sender"] == "Alice" and 9 > tempmessage["turn"] > 0:
			tempblock = tempmessage["message"]
			count = tempmessage["turn"]
			tempblock = writeBlock(tempblock)
			if tempblock != False:	
				confirmPlacement(tempblock)
				print(f'Block {count} received from Alice validated and recorded successfully')
		...
		elif tempmessage["sender"] == "Alice" and  tempmessage["turn"] == 9:
			tempblock = tempmessage["message"]
			count = tempmessage["turn"]
			tempblock = writeBlock(tempblock)
			if tempblock != False:
